
export const newsData = [
  {
    id: "1",
    title: "PKBM Gelar Wisuda Tahunan",
    excerpt: "Lebih dari 100 siswa mengikuti prosesi wisuda yang berlangsung khidmat...",
    content: `Acara wisuda tahunan PKBM Zahratul Syitha dilangsungkan pada 12 Juni 2025 dengan penuh khidmat. 
Acara ini diikuti oleh lebih dari 100 siswa dari berbagai jenjang pendidikan. 
Dalam sambutannya, Kepala PKBM menyampaikan rasa bangga terhadap capaian siswa serta berharap agar ilmu yang diperoleh dapat diaplikasikan di tengah masyarakat.

Wisuda ini juga dihadiri oleh perwakilan Dinas Pendidikan dan tokoh masyarakat setempat yang memberikan motivasi dan apresiasi kepada seluruh peserta didik.`,
    image: "http://localhost:5173/img/banner3.jpg",
    date: "12 Juni 2025",
    category: "Pendidikan",
    author: "Admin PKBM",
    tags: ["Wisuda", "Pendidikan", "Prestasi"]
  },
  {
    id: "2",
    title: "Pelatihan Kemandirian Santri",
    excerpt: "Meningkatkan keterampilan praktis peserta didik dalam dunia usaha...",
    content: `PKBM Zahratul Syitha kembali menggelar pelatihan kemandirian bagi para santri yang bertujuan untuk meningkatkan keterampilan wirausaha.
Pelatihan ini meliputi keterampilan kuliner, menjahit, dan digital marketing.

Instruktur dihadirkan dari kalangan profesional yang telah berpengalaman di bidang UMKM. 
Peserta terlihat antusias dalam mengikuti kegiatan ini, yang diharapkan dapat menjadi bekal dalam membangun usaha mandiri ke depan.`,
    image: "http://localhost:5173/img/banner3.jpg",
    date: "10 Juni 2025",
    category: "Pelatihan",
    author: "Tim Redaksi",
    tags: ["Pelatihan", "Santri", "UMKM"]
  },
  {
    id: "3",
    title: "Alumni PKBM Buka Usaha Mandiri",
    excerpt: "Mantan peserta kini sukses membuka usaha konveksi menengah...",
    content: `Salah satu alumni terbaik PKBM, Ibu Siti Marlina, berhasil membuka usaha konveksi skala menengah di kampung halamannya.
Dalam wawancara, ia menyampaikan bahwa ilmu yang diperoleh selama belajar di PKBM sangat membantunya memahami dasar-dasar kewirausahaan.

Usaha yang dirintisnya kini mempekerjakan 5 warga sekitar dan memproduksi seragam sekolah serta pakaian muslim. 
Hal ini menjadi bukti nyata bahwa pendidikan non-formal dapat memberikan dampak positif dalam kehidupan.`,
    image: "http://localhost:5173/img/banner3.jpg",
    date: "08 Juni 2025",
    category: "Prestasi",
    author: "Kontributor PKBM",
    tags: ["Alumni", "Kemandirian", "Wirausaha"]
  },
  {
    id: "4",
    title: "PKBM Jalin Kerja Sama dengan LSM Lokal",
    excerpt: "Kolaborasi dilakukan untuk memperluas akses pendidikan dan pelatihan...",
    content: `PKBM Zahratul Syitha resmi menjalin kerja sama dengan Lembaga Swadaya Masyarakat (LSM) lokal dalam rangka memperluas akses pendidikan.
Melalui kerja sama ini, beberapa program baru akan diluncurkan, termasuk pelatihan keterampilan perempuan dan pendidikan anak jalanan.

Kerja sama ini disambut positif oleh masyarakat dan diharapkan dapat meningkatkan angka partisipasi belajar.`,
    image: "http://localhost:5173/img/banner3.jpg",
    date: "05 Juni 2025",
    category: "Kemitraan",
    author: "Admin PKBM",
    tags: ["Kerja Sama", "LSM", "Pendidikan"]
  }
];
